...
//            button.active = entries.stream().allMatch(e -> e.inLimits);
	...
	private static class ConfigScreen extends DhScreen
	{
		...
		{
			super(getTitleKey(configBase, category));
		...
		private static String getTitleKey(ConfigBase configBase, String category)
		{
			String baseKey = configBase.modID + ".config";
			String keyWithCategory = baseKey + (category.isEmpty() ? "" : "." + category) + ".title";
			String defaultKey = baseKey + ".title";
			
			return I18n.format(keyWithCategory).equals(keyWithCategory) ? defaultKey : keyWithCategory;
		}
		...
		protected void actionPerformed(GuiButton button)
		{
			...
			{
				LOGGER.error("ConfigGUI: Error while performing action for button: " + button.id, e);
			}
		...
		protected void keyTyped(char typedChar, int keyCode)
		{
			...
			{
				LOGGER.error("ConfigGUI: Error while typing key: " + typedChar + " with code: " + keyCode, e);
			}
		...
		protected void mouseClicked(int mouseX, int mouseY, int mouseButton)
		{
			...
			{
				LOGGER.error("ConfigGUI: Error while clicking mouse at position: (" + mouseX + ", " + mouseY + ") with button: " + mouseButton, e);
			}
			for (GuiTextField field : textFieldList.keySet())
			{
		...
		private void addMenuItem(AbstractConfigType info)
		{
			...
			if (ConfigEntry.class.isAssignableFrom(info.getClass()))
			{
				...
				{
					int componentId = 0;
					GuiTextField widget = new GuiTextField(componentId, this.fontRenderer, this.width - 150 - ConfigScreenConfigs.SpaceFromRightScreen + 2, 0, 150 - 4, 20);
			...
			for (AbstractConfigType info : ConfigBase.INSTANCE.entries)
			{
				if (info.getCategory().matches(category) && info.getAppearance().showInGui)
				{
					if (list.getHoveredButton(mouseX, mouseY).isPresent())
					{
						...
						else if (I18n.hasKey(key) && (text != null && text.equals(name)))
						{
							List<String> list = new ArrayList<>();
							for (String str : I18n.format(key).split("\n"))
							{
	...
	public static class ButtonEntry
	{
		...
		public void render(int y, int x, int mouseX, int mouseY)
		{
			...
			if (text != null && (!text.contains("spacer") || button != null))
				Minecraft.getMinecraft().fontRenderer.drawString(text, 12, y + 5, 0xFFFFFF);
		}
