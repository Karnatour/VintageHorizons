...
public class ClientBlockStateColorCache
{
	...
	private static final HashSet<IBlockState> BLOCK_STATES_THAT_NEED_LEVEL = new HashSet<>();
	...
	private static final EnumFacing[] COLOR_RESOLUTION_DIRECTION_ORDER = {EnumFacing.UP, EnumFacing.NORTH, EnumFacing.EAST, EnumFacing.WEST, EnumFacing.SOUTH, EnumFacing.DOWN};
	...
	private void resolveColors()
	{
		...
		try
		{
			...
			if (!this.blockState.getMaterial().isLiquid())
			{
				...
				for (EnumFacing direction : COLOR_RESOLUTION_DIRECTION_ORDER)
				{
					quads = this.getQuadsForDirection(direction);
					if (quads != null && !quads.isEmpty() && !(this.blockState.getBlock() instanceof BlockRotatedPillar && direction == EnumFacing.UP))
					{
						break;
					}
				}
