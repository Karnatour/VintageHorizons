...
public class ChunkWrapper implements IChunkWrapper
{
	...
	public int getMinNonEmptyHeight()
	{
		...
		// Default to 0 if every section is empty or null
		this.minNonEmptyHeight = 0;
		
		ExtendedBlockStorage[] storageArray = this.chunk.getBlockStorageArray();
		
		// Each section is 16 blocks high
		for (int i = 0; i < storageArray.length; i++)
		{
			ExtendedBlockStorage section = storageArray[i];
			if (section != null && !section.isEmpty())
			{
				this.minNonEmptyHeight = i * 16;
				break;
			}
		}
	...
	public int getMaxNonEmptyHeight()
	{
		...
		this.maxNonEmptyHeight = 0;
		
		ExtendedBlockStorage[] sections = this.chunk.getBlockStorageArray();
		
		// Loop from top to bottom to find the highest non-empty section
		for (int i = sections.length - 1; i >= 0; i--)
		{
			ExtendedBlockStorage section = sections[i];
			if (section != null && !section.isEmpty())
			{
				this.maxNonEmptyHeight = (i + 1) * 16; // top Y of the section
				break;
			}
		}
	...
	{
		World world = (World) wrappedLevel.getWrappedMcObject();
		
		return BiomeWrapper.getBiomeWrapper(this.chunk.getBiome(new BlockPos(relX,relY,relZ),world.provider.getBiomeProvider()),wrappedLevel);
	}
	 ...
	 * // replacement getters
	 *
	 * @Override public IBlockStateWrapper getBlockState(int relX, int relY, int relZ)
	 * {
	 * this.throwIndexOutOfBoundsIfRelativePosOutsideChunkBounds(relX, relY, relZ);
	 * return this.getBlockStateInternal(relX, relY, relZ, null);
	 * }
	 * @Override public IBlockStateWrapper getBlockState(int relX, int relY, int relZ, IMutableBlockPosWrapper mcBlockPos, IBlockStateWrapper guess)
	 * {
	 * this.throwIndexOutOfBoundsIfRelativePosOutsideChunkBounds(relX, relY, relZ);
	...
	@Override
	public int getMaxBlockX() { return this.chunk.getPos().x * 16 + 16; }
	@Override
	public int getMaxBlockZ() { return this.chunk.getPos().z * 16 + 16; }
	@Override
	public int getMinBlockX() { return this.chunk.getPos().x * 16; }
	@Override
	public int getMinBlockZ() { return this.chunk.getPos().z * 16; }
				...
				for (int z = 0; z < 16; z++) {
					for (int y = 0; y < 256; y++) {
						...
						if (block.getLightValue() > 0) {
							this.blockLightPosList.add(new DhBlockPos(pos.getX(), pos.getY(), pos.getZ()));
						}
					}
				}
			}
	...
	@Override
	public String toString() { return this.chunk.getClass().getSimpleName() + this.chunk.getPos().x + "," + this.chunk.getPos().z; }
